...
| A9 | XHS 重新登录：xhs_login.py smoke test + 真实数据验证 | — | P1 | A6 | ✅ 已完成 | 36 条真实 XHS TrendSignal（TREND_20260517_XHS_*）；smoke test xfail 区分 session 过期 vs 搜索阻断 |
| A10 | Douyin CDP search() 完整实现（3h 预算） | — | P1 | A3 | ⚠️ 待配置 | search() 实现完整；需 Chrome --remote-debugging-port=9222 + Douyin 登录态；scripts/douyin_login.sh 已提供 |
| A11 | Instagram instaloader 接入 | — | P2 | A3 | ⚠️ 待登录 | _resolve_session_file() 自动发现 session；_fetch_instaloader() 已修正用户名提取；需运行 scripts/ig_login.py 生成 session |
